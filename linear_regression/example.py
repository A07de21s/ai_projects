...
print("y: %f" % sess.run(y, feed_dict={x: [[109]], x2: [[120]]}))
#输出最终的W,W2,b和cost值
print("W_Value: %f" % sess.run(W),"W2 Value: %f" % sess.run(W2),"b_Value: %f" %
        sess.run(b),"cost_Value: %f" % sess.run(cost,feed_dict=feed))
